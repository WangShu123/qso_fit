...
function Fe_flux_mgii, xval, pp
   ...
   if n_elements(wave_Fe_mgii) eq 0 then begin
     ...
     file = '/Users/legolason/study/otherwork/liaowt/spectra/mc_fit/feconv_uv_shen.txt'
...
function Fe_flux_balmer, xval, pp
   ...
   if n_elements(wave_Fe_balmer) eq 0 then begin
    ...
    ; read in the Boroson&Green optical template for Balmer
      file = '/Users/legolason/study/otherwork/liaowt/spectra/mc_fit/irontemplate.dat'
...
pro rm_qsofit, lam0, flux0, err0, z, ivar0=ivar0,ra=ra,dec=dec, deredden=deredden $
       ...
       , xtol=xtol,ftol=ftol,fwhm=fwhm,flux3000=flux3000
   ...
   if keyword_set(emparfile) then emline_file=emparfile else $
    emline_file = '/Users/legolason/study/otherwork/2liaowt/spectra/mc_fit/qsoline_all.par'
   ...
   if keyword_set(psplot) then begin
	 ...
	 set_plot, 'PS'
         device, filename = figfile, /color, xsize = 30, ysize = 20, /ENCAPSULATED
         ;begplot, name=figfile, xsize = 40, ysize = 25,/color
         ;begplot, name=figfile, /landscape, /color   ; commoned since begplot has unfinded fun
 	 ;device, /close_file
   ...
   ind_s = where(lam0 ge 1300. and lam0 le 9900.)
   ...
   flux3000=mean((f_pl_model+f_poly_model)[where(wave ge 2999. and wave le 3001.  )])
   oplot, wave, f_fe_mgii_model, color=cgcolor('blue')
   oplot, wave, f_fe_balmer_model, color=cgcolor('blue')
   oplot, wave, f_bc_model, color=cgcolor('cyan')
   for jj=0L, (size(window_all))[2] - 1 do begin
     oplot,[window_all[0,jj],window_all[0,jj]],[0.9, 0.8]*yrange[1],color=cgcolor('gray')
     oplot,[window_all[1,jj],window_all[1,jj]],[0.9, 0.8]*yrange[1],color=cgcolor('gray')
     oplot,[window_all[0,jj],window_all[1,jj]],[0.9, 0.9]*yrange[1],color=cgcolor('gray')
   endfor
